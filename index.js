      ...
      https.get(config.dark_sky + '/' + pet.pet.location.coordinates[0] + ',' + pet.pet.location.coordinates[1], (res) => {
        ...
        res.on('end', function() {
          ...
          response.render('pages/show', {
            pet: pet.pet,
            weather: (weather && weather.currently && weather.currently.icon) || (weather && weather.error),
            errors: false
          });
        });

      }).on('error', (e) => {
        response.render('pages/show', {pet: {}, weather: '', errors: 'Server Error'});
      });
    }).on('error', (e) => {
      response.render('pages/show', {pet: {}, weather: '', errors: 'Server Error'});
    })
